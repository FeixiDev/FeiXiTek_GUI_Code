...
try:
    ...
    def check_mirror_status(engine):
        '''
        Check if all mirror members of given engine are OK status
        @engine: engine index
        @-type engine: int
        @return: overall result of mirrors'(All OK", "", "#MirrorID: #MirrorMember-#Status...")
        @type return: string
        
        #Mirror ID = mlun
        #Mirror Member:engine_info[engine]['Mirror'][mlun][3,5,7,9]
        #Member Status: engine_info[engine]['Mirror'][mlun][4,6,8,10]
        '''
    ...
    print "sorry! The Mirror is go to bad"


@app.route('/Engine_full')
def Engine_full():
    return render_template("Engine_full.html")
...
def home():
    if request.method == "POST":
        ...
        if username == '1':
            ...
            del ErrList['Queue full'][:]
            del ErrList['ABTS Error'][:]
            del ErrList['SwitchError'][:]
        print "change SIGNAL:", SIGNAL[0]
...
#         print Switch_info[i]["SwitchIP"]
#         SwitchIP_list.append(Switch_info[i]["SwitchIP"])
#         
#         SwitchIP_list2 = list(set(SwitchIP_list))
#         SwitchIP_list2 = sorted(SwitchIP_list2)
#         
    ...
    for i in range(len(Switch_info)):
        for port in sorted(Switch_info[i]["PortInfo"].keys()):
           ...
           # Switch_Port_list2 = list(set(Switch_Port_list))
...
# 即时错误信息的获取
    ...
    for i in ErrList['EngineReboot']:
        ErrList_list += i + " | "
#     ErrList_list = ErrList['EngineReboot']
    # print 'ErrList_list:',ErrList_list 
    for i in ErrList['ABTS Error']:
        ABTSs_list += i + " | "
         
    for i in ErrList['Queue full']:
        QFulls_list += i + " | "
...
# aaaaa
        ...
        "ABTSs_list":ABTSs_list,
        "QFulls_list":QFulls_list,
        "SE_list":SE_list,
        }
