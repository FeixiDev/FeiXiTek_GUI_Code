...
#aaaaaaa
ABTS_signal = int(cfg['ABTS and queue_full']['ABTS_alarm'])
Qfull_signal = int(cfg['ABTS and queue_full']['Qfull_alarm'])
signal = 0
ABTS_lis = []
Time_number = []
errIP_lis = [[],[],[]]
tracepath = './HistoryFile/'
ABTS_filename = 'ABTS.txt'
 ...
 #aaaaaaa       
    def ABTS_accumulative(self,i):      
        ...
        for ABTS_value in ABTS_values:
            ABTS_alarm = ABTS_value[0] + ABTS_alarm
            Qfull_alarm = ABTS_value[1] + Qfull_alarm

        if ABTS_alarm > ABTS_signal:
            ABTS_trigger = 1
            errIP_lis[1].append(E_IP_lis[0])

        if Qfull_alarm > Qfull_signal:
            ABTS_trigger = 1
            errIP_lis[2].append(E_IP_lis[0])
...
#aaaaaa
    def errIP(self):
        if errIP_lis[0]:
            ...
            ErrList['EngineReboot'] = errIP_lis[0][:]
            self.list_initial(errIP_lis[0])

        if errIP_lis[1]:
            self.list_initial(ErrList['ABTS Error'])
            ErrList['ABTS Error'] = errIP_lis[1][:]
            self.list_initial(errIP_lis[1])

        if errIP_lis[2]:
            self.list_initial(ErrList['Queue full'])
            ErrList['Queue full'] = errIP_lis[2][:]
            self.list_initial(errIP_lis[2])
